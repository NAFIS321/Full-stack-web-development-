...
app.post("/add", async (req, res) => {
  ...
  try {
    ...
    const result = await db.query(
      ...
      [input.toLowerCase()]                // Convert input to lowercase for matching
    );
    ...
    try {
      ...
      // Error occurs if country already exists in visited_countries (duplicate key)
      console.log(err);
      const countries = await checkVisisted();
      res.render("index.ejs", {
        countries: countries,
        total: countries.length,
        error: "Country has already been added, try again.", // Show error message in view
      });
    }
  } catch (err) {
    ...
    res.render("index.ejs", {
      countries: countries,
      total: countries.length,
      error: "Country name does not exist, try again.", // Show error message in view
    });
  }
});
