...
app.get("/random", (req, res) => {
  ...
  // jokes[randomIndex] accesses random joke.
  res.json(jokes[randomIndex]);
});
...
app.get("/jokes/:id", (req, res) => {
  ...
  // It returns FIRST matching joke.
  const foundJoke = jokes.find(

    // Checking if joke.id equals requested ID.
    (joke) => joke.id === id
  );

  // Sending found joke back as JSON.
  res.json(foundJoke);
});
...
app.get("/filter", (req, res) => {
  ...
  const filteredActivities = jokes.filter(
    ...
    // Keep jokes where jokeType matches.
...
app.post("/jokes", (req, res) => {
  ...
  const newJoke = {
    ...
    // req.body.text comes from form/request body.
    jokeText: req.body.text,
  ...
  // push() adds new joke into array.
  jokes.push(newJoke);

  // slice(-1) returns last element of array.
  // Used for debugging.
  console.log(jokes.slice(-1));
...
// PUT replaces ENTIRE joke.
app.put("/jokes/:id", (req, res) => {

  // Getting ID from URL.
  const id = parseInt(req.params.id);

  // Creating completely new replacement object.
  const replacementJoke = {
  ...
  // findIndex() returns array position.
...
app.patch("/jokes/:id", (req, res) => {
  ...
  const replacementJoke = {
    ...
    // Same logic for joke type.
    jokeType:
      req.body.type || existingJoke.jokeType,
  };

  // Finding joke index in array.
  const searchIndex = jokes.findIndex(
    (joke) => joke.id === id
  );

  // Updating joke inside array.
  jokes[searchIndex] = replacementJoke;

  // Logging updated joke.
  console.log(jokes[searchIndex]);
...
app.delete("/all", (req, res) => {
  ...
  // Checking if key matches master key.
  if (userKey === masterKey) {

    // Emptying entire jokes array.
    jokes = [];
